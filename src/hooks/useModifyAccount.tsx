...
export function useModifyAccount() {
    ...
    useEffect(() => {
        if(bancosDb.length > 0 && form.banco !== 0){
            setShowInicio(form.banco)
        }
    }, [form, bancosDb])
    ...
    const handleContinueForm = async () => {
        ...
        setIsLoading(true);
        try {
            const { success } = await PerfilService.postMiCuenta({
                banco: form.banco,
                cuenta: form.cuenta,
    ...
    const setShowInicio = (value: number) => {
        const bancoFilter = bancosDb.filter((itemBanco) => itemBanco.id === value)
    ...
    const setBanco = (value: string) => {
        ...
        console.log(bancoFilter, bancosDb, value)
        if (bancoFilter.length > 0) {
            const bancoItem = bancoFilter[0];
            if (bancoItem.ap_tipo_cuenta) {
                setShowTipoCuenta(true)
